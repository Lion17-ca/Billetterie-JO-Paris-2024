#!/usr/bin/env python3
"""
Script pour créer un compte utilisateur normal dans la base de données.
"""
...
class User(Base):
    ...
    is_employee = Column(Boolean, default=False)     # Utilisateur normal, pas un employé
...
def create_regular_user(email, password, first_name, last_name):
    ...
    try:
        ...
        new_user = User(
            ...
            is_employee=False,  # Utilisateur normal
            is_admin=False,     # Pas un administrateur
            is_active=True
        )
        ...
        print(f"Compte utilisateur créé avec succès : {email}")
        return new_user
...
if __name__ == "__main__":
    ...
    print(f"Création d'un compte utilisateur normal avec les informations suivantes :")
    print(f"Email: {email}")
    print(f"Prénom: {first_name}")
    print(f"Nom: {last_name}")
    
    # Créer le compte utilisateur
    create_regular_user(email, password, first_name, last_name)
